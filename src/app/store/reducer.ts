 ...
 * @param {MesMatchsAction.All} action
 * @returns {State}
 */
export function reducer(state = initialState, action: BeerActions.All): AppState {
  switch (action.type) {
    case BeerActions.ADD_BEER: {
      const beer = action.payload;
      beer.id = state.beers.length;
